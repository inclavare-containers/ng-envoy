...
LibratsTlsCertificateConfigProviderImpl::LibratsTlsCertificateConfigProviderImpl(
    ...
    : tls_certificates_librats_config_(
          std::make_unique<envoy::extensions::transport_sockets::tls::v3::LibratsCertificate>(tls_certificates_librats_config))
          ,tls_certificate_(std::make_unique<envoy::extensions::transport_sockets::tls::v3::TlsCertificate>())
          {
            ...
            //tls_certificate_ = std::make_unique<envoy::extensions::transport_sockets::tls::v3::TlsCertificate>(std::move(&tls_certificate));
        }

void generate_certificate(uint8_t **private_key, size_t *private_key_size, uint8_t **certificate_out, size_t *certificate_size_out) {
    ...
    X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, reinterpret_cast<const unsigned char *>("example.com"), -1, -1, 0);
...
const envoy::extensions::transport_sockets::tls::v3::TlsCertificate* LibratsTlsCertificateConfigProviderImpl::secret() const {
    //in
    ...
    if(tls_certificates_librats_config_->attester() == "tdx"){
        //std::strncpy(conf.attester_type, tls_certificates_librats_config_->attester().c_str(), sizeof(conf.attester_type));
    ...
    std::cout <<"custom_claim_size: " << tls_certificates_librats_config_->custom_claims_size() << std::endl;
    ...
    ret = librats_get_attestation_certificate(conf,subject_name,&private_key,&private_key_size,claims,claims_length,provide_endorsements,&certificate_out,&certificate_size_out);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    [[maybe_unused]] const uint8_t* const_certificate = reinterpret_cast<const uint8_t*>(cert_chain_->inline_bytes().data());
    std::cout <<"RE certificate: " <<std::endl;
    for(size_t i = 0; i < certificate_size_out; i++) {
        std::cout << certificate_out[i];
    }  
    ...
    */
    ...
    //[[maybe_unused]] const uint8_t* const_private_key = reinterpret_cast<const uint8_t*>(private_key_->inline_bytes().data());
